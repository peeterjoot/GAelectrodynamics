...
For our purposes, a manifold is defined by a parameterization, such as \( \Bx(a,b) \) where \( a, b\) are scalar parameters.  With one parameter the vector traces out a curve, with two a surface, three a volume, and so forth.
...
where \( j \) is an index not a power\footnote{In tensor algebra, the index \( j \), which occurs in matched upper and lower index pairs, is known as a dummy summation index.
Such matched pairs imply a freedom to 
utilize any unused index when convenient, such as \( \sum_k a^k \Bx_k \), but this is obvious when summation symbols are included explicitly.}.
...
In tensor algebra, a vector, identified by the coordinates \( a^i \) is called a covariant vector.  
When that vector is identified by the coordinates \( a_i \) it is called a contravarient vector.  These labels relate to how the coordinates transform with respect to norm preserving transformations.
