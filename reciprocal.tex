...
%\makeaside{Upper and lower indexes}{aside:reciprocal:1}
\makedigression{
...
Generally, when summation convention is used, explicit summation is only used explicitly when upper and lower indexes are not perfectly matched, but summation is still implied.  Readers of texts that use summation convention can check for proper matching of upper and lower indexes to ensure that the expressions make sense.  For this reason a mixed index Kronecker delta has been used in the definition above.

Summation convention will not be used explicitly in this text, as it deviates from normal practises in electrical engineering.
}

The concept of a reciprocal frame generalizes the notion of normal to non-orthonormal bases.
%
%\paragraph{Motivation:}
Reciprocal frames are required to express the GA form of Stokes' theorem and for more general GA integration theory, including integration with respect to both curvilinear coordinates and higher dimensions.
...
Dotting with the reciprocal frame vectors \( \Bx^i \) then provides the coordinates \( a^i \) of the vector with respect to the original basis
...
where a switch of dummy summation indexes \( i \rightarrow j \) was made to avoid conflict.
