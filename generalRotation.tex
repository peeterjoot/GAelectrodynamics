...
and that the sign of the 2-blade exponential parameter must be negated to maintain the value of the vector \( \Bx \in \bbR^2 \) on interchange
...
\end{equation*}
\item
Reversing the order of multipication of a
vector \( \Bp \in S \) with an exponential 
\( e^{ \Ba \wedge \Bb } \), requires the sign of the exponential arguement to be negated
\begin{equation*}
...
\item
Any normal vectors \( \Bn \) commute with a such a complex exponential
\begin{equation*}
...
Let \( B = \setlr{ \pcap, \qcap } \) be an orthonormal basis for a planar subspace with unit pseudoscalar \( i = \pcap \qcap \) where \( i^2 = -1\).  The rotation of a vector \( \Bx \) through an angle \( \theta \) with respect to this plane is
\begin{equation*}
...
This statement did not make any mention of a normal direction.
...
rotates components of the vector that lie in the planar subspace \( S \) by \( \theta \), while leaving the normal components of the vector unchanged, as illustrated in \cref{fig:Rotation:RotationFig1}.  This is what we can call rotation around a normal in \R{3}.

\imageFigure{../figures/GAelectrodynamics/RotationFig1}{Rotation with respect to a plane.}{fig:Rotation:RotationFig1}{0.5}
