...
\maketheorem{Commutation and conjugation rules for wedge products and complex exponentials.}{thm:SimpleProducts2:1780}{
Given two
...
Let \( B = \setlr{ \pcap, \qcap } \) be an orthonormal basis for a planar subspace with unit pseudoscalar \( i = \pcap \qcap \).  The rotation of a vector \( \Bx \) through an angle \( \theta \) with respect to this plane is
...
\begin{dmath}\label{eqn:generalRotation:n}
R_\theta(\Bx) 
= 
...
This rotation operation rotates components of the vector that lie in the planar subspace \( S \) by \( \theta \), and leaves any normal components of the vector unchanged.
