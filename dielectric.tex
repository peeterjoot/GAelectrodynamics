...
The space time gradient equations of \cref{thm:dielectric:200} can be solved for \( F', G' \) using Green's function convolution.  If we can impose a zero scalar grade constraint on \( F' \) and a zero pseudoscalar grade constraint on \( G' \) then we have figured out how to solve \cref{thm:dielectric:20}.

\paragraph{Question:} Would a grade selection of the convolution work?
