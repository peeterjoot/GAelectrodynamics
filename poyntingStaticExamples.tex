...
Calculation of the \( T(\Be_k) \) tensor components is generally more involved.  Let's do this calculation for each of the fields above in turn to illustrate.
...
T(\Be_k)
= - \frac{\BS}{c} \cdot \Be_k + \gpgradeone{T(\Be_k)},
\end{dmath}
...
\begin{aligned}
\gpgradeone{T(\rhocap)} &= \frac{\epsilon}{2} E^2 \lr{1 -\Bv^2/c^2} \rhocap \\
\gpgradeone{T(\thetacap)} &= -\frac{\epsilon}{2} E^2 \lr{1 -\Bv^2/c^2} \thetacap \\
%\gpgradeone{T(\Be_1)} &= \frac{\epsilon}{2} E^2 (1 -\Bv^2/c^2) \Be_1 e^{2 i \theta} \\
%\gpgradeone{T(\Be_2)} &= -\frac{\epsilon}{2} E^2 (1 -\Bv^2/c^2) \Be_2 e^{2 i \theta} \\
\gpgradeone{T(\Be_3)} &= -\frac{\epsilon}{2} E^2 \lr{1 + \Bv^2/c^2 } \Be_3.
\end{aligned}
\end{dmath}

For this field that \( \gpgradeone{T(\rhocap)} \) is entirely radial, whereas \( \gpgradeone{T(\thetacap)} \) is entirely azimuthal.
...
So the stress tensor components of interest are
