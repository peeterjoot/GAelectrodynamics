...
\end{mmaCell}
%inverse[a_] := \mmaFrac{a}{GeometricProduct[a, a]};

This gives \( \Bx^1 = 2 \Be_1 - \Be_2, \Bx^2 = \Be_2 - \Be_1 \), which 
satisfies the defining property
\( \Bx_i \cdot \Bx^j = {\delta_i}^j \).
%\begin{mmaCell}[moredefined={InnerProduct, x1, s1, x2, s2},morepattern={\#1}]{Input}
%  (InnerProduct[#1[[1]],#1[[2]]]&)/@\{\{x1,s1\},\{x2,s2\},\{x1,s2\},\{x2,s1\}\}
%\end{mmaCell}
%} % example
