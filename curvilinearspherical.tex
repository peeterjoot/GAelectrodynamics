...
This curvilinear basis can be easily be computed in software
\begin{mmaCell}[moredefined={i, j, ej, x, xr, xt, xp, e, \
GeometricProduct},morepattern={phi_, t_, p_, r_, theta_}]{Input}
...
Unfortunately the compact representation is lost doing so.  Computing them manually, we find
...
The scalar factor is in fact the Jacobean with respect to the spherical parameterization
...
The final reduction of \cref{eqn:curvilinearspherical:300}, and the expansion of the Jacobean
\cref{eqn:curvilinearspherical:320} both easily submit to software evaluation

\begin{mmaCell}[moredefined={OuterProduct, xr, xt, xp, e1, e2, e3, jacobean, e}]{Input}
  OuterProduct[ xr[r, \mmaUnd{\(\pmb{\theta}\)}, \mmaUnd{\(\pmb{\phi}\)}],
  xt[r, \mmaUnd{\(\pmb{\theta}\)}, \mmaUnd{\(\pmb{\phi}\)}],
  xp[r, \mmaUnd{\(\pmb{\theta}\)}, \mmaUnd{\(\pmb{\phi}\)}]]

  \{e1,e2,e3\} = IdentityMatrix[3];
  jacobean = \{xr[r, \mmaUnd{\(\pmb{\theta}\)}, \mmaUnd{\(\pmb{\phi}\)}],
  xt[r, \mmaUnd{\(\pmb{\theta}\)}, \mmaUnd{\(\pmb{\phi}\)}],
  xp[r, \mmaUnd{\(\pmb{\theta}\)}, \mmaUnd{\(\pmb{\phi}\)}]\} /. \{e[1] \(\pmb{\to}\) e1, e[2] \(\pmb{\to}\) e2, e[3]\(\pmb{\to}\)e3\};
  Det[  jacobean ] // Simplify
\end{mmaCell}
\begin{mmaCell}{Output}
...
Performing these calculations manually is left as a problem
for the student (\cref{problem:curvilinearspherical:1}).

\makeproblem{Spherical volume Jacobean.}{problem:curvilinearspherical:1}{
Perform the final reduction of \cref{eqn:curvilinearspherical:300}, and
