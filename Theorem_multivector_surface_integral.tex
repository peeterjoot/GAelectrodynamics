%
% Copyright © 2018 Peeter Joot.  All Rights Reserved.
% Licenced as described in the file LICENSE under the root directory of this GIT repository.
%
\maketheorem{Multivector surface integral.}{thm:surfaceintegral:100}{
Given an connected surface \( S \) with an area element \( d^x \Bx = d\Bx_1 \wedge d\Bx_2 \), and multivector functions \( F, G \), the \textit{surface integral} is related to the boundary by
\begin{equation*}
\int_S F d^2\Bx \lrboldpartial G
= \ointclockwise_{\partial S} F d\Bx G,
\end{equation*}
where \( \partial S \) is the boundary of the surface \( S \).
} % theorem
