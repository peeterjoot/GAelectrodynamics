...
\Cref{eqn:magnetostatics:640} states that the magnetic flux through a closed surface is zero, which is not be a surprise since it is a direct consequence of Gauss's law \( \spacegrad \cdot \BB = 0 \).  \Cref{eqn:magnetostatics:660} provides a relationship between the tangential components of the magnetic field and the total enclosed current density.
...
%\begin{equation}\label{eqn:magnetostatics:780}
\boxedEquation{eqn:magnetostatics:800}{
\begin{aligned}
\ointclockwise_{\partial S} d\Bl \cdot \BB &= \mu_0 \int_S d^2 \Bx \cdot (I \BJ) \\
\ointctrclockwise_{\partial S} d\Bl \cdot \BB &= \mu_0 \int_S dA \ncap \cdot \BJ.
\end{aligned}
}
%\end{equation}
