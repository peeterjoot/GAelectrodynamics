...
Let \( \Bu \) and \( \Bv \) be an orthonormal pair of vectors in the plane of \( \Ba \) and \( \Bb \), oriented in a positive rotational sense as illustrated in
...
With respect to the orthonormal vectors \( \Bu \) and \( \Bv \), a
a polar representation of \( \Ba, \Bb \) is
\begin{dmath}\label{eqn:products:1660}
\begin{aligned}
\Ba &= \Norm{\Ba} \Bu e^{ i_{ab} \theta_a } = \Norm{\Ba} e^{ -i_{ab} \theta_a } \Bu \\
\Bb &= \Norm{\Bb} \Bu e^{ i_{ab} \theta_b } = \Norm{\Bb} e^{ -i_{ab} \theta_b } \Bu,
\end{aligned}
\end{dmath}
where \( i_{ab} = \Bu \Bv \) is a unit pseudoscalar for the planar subspace spanned by \( \Ba \) and \( \Bb \).
The vector product of these two vectors is
\begin{dmath}\label{eqn:products:1680}
\Ba \Bb
=
\lr{ \Norm{\Ba} e^{ -i_{ab} \theta_a } \Bu } \lr{ \Norm{\Bb} \Bu e^{ i_{ab} \theta_b } }
=
 \Norm{\Ba} \Norm{\Bb}
e^{ -i_{ab} \theta_a } ( \Bu \Bu ) e^{ i_{ab} \theta_b }
=
 \Norm{\Ba} \Norm{\Bb}
...
The scalar grade of the vector product is the \R{N} dot product, but the grade 2 component of the vector product is something new that requires a name.
%We respectively identify and define operators for these vector grade selection operations as 
...
be used with \( \Ba = \spacegrad \) to combine (the scalar) Gauss's law with (the vector) Maxwell-Faraday equation, and to
