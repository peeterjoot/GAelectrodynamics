...
Recall that the electromagnetic field, with \( \BE = \BE_r + j \BE_i \), where \( \BE_r \cdot \kcap = \BE_i \cdot \kcap = 0 \), for a plane wave is
...
\paragraph{Circular and linear polarized waves.}
...
Observe that for this choice of pseudoscalar, the grade selection is a no-op, so the electromagnetic field is just
...
This particular example is that of a circularly polarized state.  The linear polarized plane wave states can be recovered by superposition.  For example, again with \( \kcap = \Be_3, i = \Be_1 \Be_2 \),
