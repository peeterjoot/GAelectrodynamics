...
In a 2D space most of the interesting vector products involve the unit bivector \( i = \Be_1 \Be_2 \).
...
In complex algebra, multiplication by \( \pm i \) will rotate a complex number \( z \) by \( \pm \pi/2 \) radians.
The plane pseudoscalar rotates plane vectors in a similar fashion, however, the rotation direction depends on whether left of right multiplication is used.

FIXME: rework from here.


A product of a spanning set of normal vectors for a space (or subspace) is called a pseudoscalar for that space (or subspace).
The particular pseudoscalar \( \Be_1 \Be_2 \) will be labelled \textbf{the pseudoscalar}.
Computation shows that multiplication with the pseudoscalar anticommutes with the 2D basis vectors
