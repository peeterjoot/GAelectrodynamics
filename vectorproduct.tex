...
In \R{3} the vector product can be put into an exponential form that combines the dot and cross products into a single expression.  With

\begin{dmath}\label{eqn:vectorproduct:340}
\begin{aligned}
\Bx \cdot \By &= \cos\theta \\
\Bx \cross \By &= \ncap \sin\theta,
\end{aligned}
...
application of \cref{eqn:vectorproduct:100} gives
\begin{dmath}\label{eqn:vectorproduct:360}
\Bx \By
=
\Abs{\Bx}
\Abs{\By}
...
\boxedEquation{eqn:vectorproduct:380}{
\Bx \By
=
\Abs{\Bx}
\Abs{\By}
...
This is possible since the dual of the normal, \( I \ncap \), is a bivector representing the plane of rotation from \( \Bx \) to \( \By \), and acts as an imaginary
...
Observe the similarity between this and the complex inner product \( z w^\conj = r \rho e^{i(\theta-\alpha)} \) for the complex numbers of \cref{eqn:GAmotivation:200}.  The imaginary in this geometric product exponential has an orientation that the complex imaginary does not.

This identification of the dot and wedge products by \R{3} analogy is not generally satisfactory.
To gain a better understanding of the geometry involved consider the components of a vector \( \Bb \) onto the direction of \( \Ba \) and the perpendicular.  The projection component is
...
These are plotted in \cref{fig:projectionAndRejection:projectionAndRejectionFig1}.
...
The product of the respective unit vectors is
...
\begin{dmath}\label{eqn:vectorproduct:540}
...
as originally observed by analogy.

The above angular decomposition can be summarized as
