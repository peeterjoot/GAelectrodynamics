...
\end{aligned}
\end{dmath}
where \( \frac{d}{dr} = (1/c) \PDi{t}{} + \Bi \PDi{x}{} + \Bj \PDi{y}{} + \Bk \PDi{z}{} \) \citep{jack2003physical}.
...
However, this structure requires notation that is arguably non-intuitive.  The
fact that the quaterion representation was abandoned long ago by most electromagnetism researchers and engineers seems to support such an argument.

\paragraph{Minkowski tensor representation.}
Minkowski introduced the concept of a complex time coordinate \( x_4 = i c t \) for special relativity \citep{einstein2015relativityMinkowski}.  Such a four-vector representation can be used for many of the relativistic four-vector pairs of electromagnetism, such as the current \((c\rho, \BJ)\), and the energy-momentum Lorentz force relations, and can also be applied to Maxwell's equations
...
Most grievous of the costs is the requirement to use differentials \( dx^1, dx^2, dx^3, c dt \), instead of a more familar set of direction vectors, even when using a non-curvilinear basis.  This requirement is easily viewed as unnatural, and likely one of the reasons that electromagnetism with differential forms has never become popular.
...
Uncoincidentally \( F \) has exactly the same structure as the 2-form \(\alpha\) in \cref{eqn:ece2500report:180}, provided the differential 1-forms \( dx^\mu \) are replaced by the basis vectors \( \gamma_\mu \).  However, there is a simple complex structure inherent in \cref{eqn:ece2500report:200} that is not obvious in the 2-form equivalent.
...
These authors use an Euclidean basis ``vector'' \( \Be_0 = 1 \) for the timelike direction, along with a standard Euclidean basis \( \setlr{ \Be_i } \) for the spatial directions.  A hybrid scalar plus vector representation of four vectors, called paravectors is employed.
...
The aim of this work was to remove conceptual roadblocks prerequisites that make electromagnetism using geometric algebra inaccessbile.
In particular, this project explored non-relativistic applications of geometric algebra to electromagnetism.
The form of Maxwell's equations that is the basis of this work
is
that of
the
paravector formalism, as stated in
\cref{eqn:ece2500report:120}.
However,
most of the paravector literature is focused on relativistic applications, and it is not accessible to engineers or engineering students.
In particular, paravector literature includes a confusing set of conjugation operations that are tailored for relativisitic applications that are not neccessary
for low velocity applications.

The end product of this project was a fairly small self contained book including an introduction to Euclidean geometric algebra based on \R{2} and \R{3} (64 pages), an introduction to geometric calculus and multivector Green's functions (64 pages), and applications to electromagnetism (75 pages).
...
This includes many of the existing results from the geometric algebra STA and paravector literature, and some new ones.
...
For example, some authors use special notation for the bivector valued magnetic field \( I \BB \), such as \( \bcap \) or \( \Bcap \).  Given the inconsistencies in the literature, \( I \BB \) (or \( I \BH \)) will be used explicitly for the bivector (magnetic) components of the total electromagnetic field \( F \).
