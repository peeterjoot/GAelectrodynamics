...
\gpgradezero{\spacegrad \cdot \lr{ \epsilon \BF}} = \rho_s \delta(y)
\end{dmath}
\begin{dmath}\label{eqn:boundary:240}
\gpgradeone{\spacegrad \cdot \lr{ \inv{\eta} \BF}} = -j \frac{k}{\eta} \BE - \BJ_s \delta(y)
\end{dmath}
\begin{dmath}\label{eqn:boundary:260}
\gpgradezero{\spacegrad \cdot \lr{ I \inv{v}\BF}} = - \rho_{ms} \delta(y)
\end{dmath}
\begin{dmath}\label{eqn:boundary:280}
\gpgradeone{\spacegrad \cdot \lr{ I \BF}} = j k \eta \BH + \BM_s \delta(y).
\end{dmath}
\end{subequations}

Each of the grade selections picks off one of \( \BD, \BB, \BH \) or \( \BE \), so this could have been obtained directly from the conventional set of individual Maxwell equations, however, it is instructional to see how to work with the complete electromagnetic field \( \BF \).  This also provides a method of evaluating the boundary conditions that is both coordinate free, and uses the same integral form for all the boundary conditions.
...
\gpgradezero{\int dV \ncap \cdot \lr{ \epsilon \BF}} = \Delta A \rho_s
\end{dmath}
\begin{dmath}\label{eqn:boundary:340}
\gpgradeone{\int dV \ncap \cdot \lr{ \inv{\eta} \BF}} = -j \omega \int_y dy \int dA \BD - \Delta A \BJ_s
\end{dmath}
\begin{dmath}\label{eqn:boundary:360}
\gpgradezero{\int dV \ncap \cdot \lr{ I \inv{v}\BF}} = - \Delta A \rho_{ms}
\end{dmath}
\begin{dmath}\label{eqn:boundary:380}
\gpgradeone{\int dV \ncap \cdot \lr{ I \BF}} = j \omega \int_y dy \int dA \BB + \Delta A \BM_s
\end{dmath}
\end{subequations}
...
\begin{aligned}
\gpgradezero{\ncap (\BF_2 -\BF_1) } &= \rho_s \\
\gpgradeone{\ncap \lr{\inv{\eta_2} \BF_2 -\inv{\eta_1} \BF_1 } } &= - \BJ_s \\
\gpgradezero{\ncap I \lr{ \inv{v_2}\BF_2 - \inv{v_1} \BF_1 } } &= - \rho_{ms} \\
\gpgradeone{\ncap I (\BF_2 - \BF_1)} &= \BM_s
\end{aligned}
}
%\end{dmath}
