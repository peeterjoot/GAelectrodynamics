...
Geometric algebra is founded on the assumption that vectors can be multiplied, and that sums of scalars, vectors, and vector products, called multivectors, are well defined.
Multivector addition and multiplication conforms to most of the rules that anybody would be expect of number like quantities, with a couple exceptions:
order that multivectors are multiplied matters, and there is also no general multiplicative inverse.
Multivector multiplication is also constrained by a rule that provides meaning for the square of a vector.
The formal statement of the algebra is that of the multivector space.
