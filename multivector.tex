...
A vector space is a set \( V \), the elements of which are called vectors, which is closed under addition and scalar multiplication operations, or

\begin{enumerate}[(a)]
   \item given any pair of vectors \( \Bx, \By \in V \) the additive result is a vector \( \Bx + \By \in V \), and
   \item given any vector \( \Bx \) and any scalar \( c \in \bbR \) (a number), the product of the two is a vector \( c \Bx \in V \).
\end{enumerate}
...
\begin{tcolorbox}[tab2,tabularx={X|Y},title=Vector space axioms.,boxrule=0.5pt]
    ...
    multiplication (by scalars) is distributive.  & \( a( \Bx + \By ) = a \Bx + a \By \), \( (a + b)\Bx = a \Bx + b\Bx \) \\ \hline
    scalar multiplication is associative. & \( (a b) \Bx = a ( b \Bx ) \) \\ \hline
    There exists a scalar multiplicative identity \( 1 \). & \( 1 \Bx = \Bx \) \\ \hline
\end{tcolorbox}
}
...
A multivector space \( M \), associated with a normed vector space \( V \), the elements of which are called multivectors, is closed under addition and multivector multiplication operations, or

\begin{enumerate}[(a)]
   \item given any pair of multivectors \( x, y \in M \), the additive result is a multivector \( x + y \in M \), and
   \item given any pair of multivectors \( x, y \in M \), the product of the two is a multivector \( x y \in M \).
\end{enumerate}

For all multivectors \( x, y, z \in M \) and vectors \( \Bx \in V \),
the addition and multiplication operations must satisfy the following axioms
...
\begin{tcolorbox}[tab2,tabularx={X|Y},title=Multivector space axioms.,boxrule=0.5pt]
    ...
    For any \( x \in M \) there exists a negative additive inverse \( -x \in M \). & \( x + (-x) = 0 \) \\ \hline
    multiplication is distributive.  & \( z( x + y ) = z x + z y \), \( (z + w)x = z x + w x \) \\ \hline
    multiplication is associative. & \( (a b) x = a ( b x ) \) \\ \hline
    There exists a scalar multiplicative identity \( 1 \). & \( 1 x = x \) \\ \hline
\end{tcolorbox}
}
...
%%
%%Given a 
%%Geometric Algebras are constructed from vectors for which a length and product operation is defined.
%%
%%
%%with an associated length and product operation.
%%
%%is based introduces a multiplication operation for vectors
%%
%%, generalizes the concept of a vector, introducing a new type of mathematical object, the \boldTextAndIndex{multivector}.  
%%A multivector is formed from sums of products of vectors.
%%GA introduces a vector multiplication operation, the vector product. 
%%Multivectors are 
%%
%%
