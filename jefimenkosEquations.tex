...
\begin{proof}
With \( \Br = \Bx - \Bx', r = \Norm{\Br} \), \( \rcap = \Br/r \) and \( \tau = t - t' \), the Green's
function from \cref{thm:greensFunctionSpacetimeGradient:120} is
\begin{equation}\label{eqn:jefimenkosEquations:15}
\begin{aligned}
G(\Br, \tau)
&=
\inv{4 \pi}
...
\end{aligned}
\end{equation}
Here we've used \cref{eqn:derivativeOfDeltaFunction:140} to evaluate the derivatives in the Green's function.  Substitution gives
\begin{equation}\label{eqn:jefimenkosEquations:20}
\begin{aligned}
F(\Bx, t)
&= F_0(\Bx, t) + \int dV' dt' \, G(\Bx - \Bx', t - t') J(\Bx', t') \\
&= F_0(\Bx, t) \\
&\quad +
\inv{4\pi}
\int dV' dt'
...
\begin{equation}\label{eqn:jefimenkosEquations:80}
\begin{aligned}
F
&=
\inv{4 \pi}
