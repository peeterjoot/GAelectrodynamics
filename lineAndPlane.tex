...
\begin{dmath}\label{eqn:lineAndPlane:60}
   \lr{ \Bx - \By } \cross \Bu = 0.
\end{dmath}
Given a vector \( \Bs \), representing a point, not neccessarily on the line, we can compute the (shortest) distance from that point to the line.
...
\end{dmath}
or
\boxedEquation{eqn:lineAndPlane:120}{
   \BD = \lr{\lr{\Bx - \Bs} \wedge \Bu } \inv{\Bu} =
   \frac{\Bu \cross \lr{\lr{ \Bx - \Bs} \cross \Bu}}{\Norm{\Bu}^2}.
}
...
\end{dmath}
This equation is satisfied by all points \( \Bx, \By \) that lie in the plane, provided we impose a constraint, either requiring that both points are not zero, or by shifting the plane so that it passes through the origin (which may not have been the case in the original parametric representation.)
...
= 0,
\end{dmath}
which demonstrates that this is the component we are interested in.  The directed (shortest) distance from the point \( \Bs \) to the plane is
